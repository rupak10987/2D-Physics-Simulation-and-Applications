...
public:
    ...
    struct VEC2 vel_dt;
    struct VEC2 acceleration= {0,9.8}; //(meter/sec^2)
...
public:
    ...
    virtual void update()
    {
        ...
        this->ball1->current_pos= {this->ball1->current_pos.X_Pos+(0.02*overlap*n_hat.X_Pos), this->ball1->current_pos.Y_Pos+(0.02*overlap*n_hat.Y_Pos)};
        this->ball2->current_pos= {this->ball2->current_pos.X_Pos-(0.02*overlap*n_hat.X_Pos), this->ball2->current_pos.Y_Pos-(0.02*overlap*n_hat.Y_Pos)};
    ...
    for(int i=0; i<4; i++)
    {
        if(i<3)
        {
            for(int j=0; j<7; j++)
            {
                ...
                number_of_links++;
                new SOFT_LINKS(v_balls[i*8+j],v_balls[(i+1)*8+j+1],20,20,all_links,number_of_links);//diagonal->'\'
                number_of_links++;
                new SOFT_LINKS(v_balls[i*8+j+1],v_balls[(i+1)*8+j],20,20,all_links,number_of_links);//diagonal->'/'
                number_of_links++;
                if(j==6)
                {
                    new SOFT_LINKS(v_balls[i*8+j+1],v_balls[((i+1)*8)+j+1],20,20,all_links,number_of_links);//vertical[right most]
                    number_of_links++;
                }
            }
        }
        else
        {
            for(int j=0; j<7; j++)
            {
                new SOFT_LINKS(v_balls[i*8+j],v_balls[i*8+j+1],20,20,all_links,number_of_links);//horizontal [bottom most]
                number_of_links++;
            }
