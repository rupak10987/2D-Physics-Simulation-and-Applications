...
public:
    ...
    virtual void update()
    {
        ...
        struct VEC2 n_hat= {to_obj.X_Pos/dist,to_obj.Y_Pos/dist};
            ...
            this->ball1->current_pos= {this->ball1->current_pos.X_Pos+(0.03*overlap*n_hat.X_Pos), this->ball1->current_pos.Y_Pos+(0.03*overlap*n_hat.Y_Pos)};
            this->ball2->current_pos= {this->ball2->current_pos.X_Pos-(0.03*overlap*n_hat.X_Pos), this->ball2->current_pos.Y_Pos-(0.03*overlap*n_hat.Y_Pos)};
...
int main()
{
    ...
    for(int i=0; i<7; i++)
    {
        if(i<7)
        {
            for(int j=0; j<7; j++)
            {
             ///v_balls[i*8+j]->rad+v_balls[((i+1)*8)+j]->rad+
            new SOFT_LINKS(v_balls[i*8+j],v_balls[((i+1)*8)+j],20,20,all_links,number_of_links);//vertical
            number_of_links++;
            new SOFT_LINKS(v_balls[i*8+j],v_balls[i*8+j+1],20,20,all_links,number_of_links);//horizontal
            number_of_links++;
            new SOFT_LINKS(v_balls[i*8+j],v_balls[(i+1)*8+j+1],20,20,all_links,number_of_links);//diagona->'\'
            number_of_links++;
            new SOFT_LINKS(v_balls[i*8+j+1],v_balls[(i+1)*8+j],20,20,all_links,number_of_links);//diagona->'/'
            number_of_links++;
            }
        }
        else
    ...
    v_balls[8]->is_static=true;
    v_balls[16]->is_static=true;
    v_balls[24]->is_static=true;
    v_balls[32]->is_static=true;
    v_balls[40]->is_static=true;
    v_balls[48]->is_static=true;
    v_balls[52]->is_static=true;
    ...
    while(true)
    {
        ...
        for(int i=0; i<num_of_balls; i++)
        {
            ...
            v_balls[0]->current_pos={280,450};
            v_balls[8]->current_pos={280,475};
            v_balls[16]->current_pos={280,500};
            v_balls[24]->current_pos={280,525};
            v_balls[32]->current_pos={280,550};
            v_balls[40]->current_pos={280,575};
            v_balls[48]->current_pos={280,600};
            v_balls[52]->current_pos={280,625};
            circle(v_balls[i]->current_pos.X_Pos,v_balls[i]->current_pos.Y_Pos,v_balls[i]->rad);
        }
        for(int j=0; j<number_of_links; j++)
        {
            if(all_links[j]->ball1!=nullptr || all_links[j]->ball2!=nullptr)
            {
            all_links[j]->update();
